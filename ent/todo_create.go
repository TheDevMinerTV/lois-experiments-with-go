...
func (tc *TodoCreate) SetItem(s string) *TodoCreate {
	tc.mutation.SetItem(s)
	return tc
}
...
func (tc *TodoCreate) check() error {
	if _, ok := tc.mutation.Item(); !ok {
		return &ValidationError{Name: "item", err: errors.New(`ent: missing required field "Todo.item"`)}
	}
	if _, ok := tc.mutation.Done(); !ok {
...
func (tc *TodoCreate) createSpec() (*Todo, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.Item(); ok {
